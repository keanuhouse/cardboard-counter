...
public class CropImageView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mBitmap != null) {
            ...
            scale_width = (float)widthSize/mBitmap.getWidth();
            scale_height = (float)heightSize/mBitmap.getHeight();
        }
...
//                    }
    ...
    // 声明native方法
    public native int[] cut_img(Bitmap bitmap);
    protected void configureBounds() {
        ...
        if (isFirst) {
            int[] result = cut_img(mBitmap);

            int left11 = (int) ((float)result[0]*scale_width);
            int top11 = (int) ((float)result[1]*scale_height);
            int right11 = (int) ((float)result[2]*scale_width);
            int bottom11 = (int) ((float)result[3]*scale_height);

            System.out.println("原图宽：" + mBitmap.getWidth());
            System.out.println("原图高：" + mBitmap.getHeight());

            System.out.println("检测left为：" + left11);
            System.out.println("检测top为：" + top11);
            System.out.println("检测right为：" + right11);
            System.out.println("检测bottom为：" + bottom11);
            ...
            // 通过对mDrawableFloat的设置来初始化框的位置
            mDrawableFloat.set(left11+10, top11, right11-10, bottom11);
